...
public class MyVoteServer extends Thread  {
	...
	public ArrayList<Boolean> getWriteIns(){
		return election.writeIns;
	}
	...
	public  void addWriteIn(String voterID, String race, Candidate cand){
		...
		election.votes.get(race).add(cand);
		System.out.println("Add write in:"+ election.votes);
		if(election.voterIDs.containsKey(voterID))
			election.voterIDs.get(voterID).add(cand);
		
		else 
		{
			List<Candidate> c = new ArrayList<Candidate>(); c.add(cand);
			election.voterIDs.put(voterID, c);
		}
		backup();
